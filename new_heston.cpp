...
double ImpliedVolatility(std::string ticker, std::string expiration){
    std::string call_options = GET(OPURL(ticker, "call", expiration));
    std::string put_options = GET(OPURL(ticker, "put", expiration));
    ptree Calls = JSON(call_options);
    ptree Puts = JSON(put_options);

    std::vector<double> price, impvol;
    for(ptree::const_iterator it = Calls.begin(); it != Calls.end(); ++it){
        ...
        if(it->first == "iv"){
            for(ptree::const_iterator jt = it->second.begin(); jt != it->second.end(); ++jt){
                impvol.push_back(atof(jt->second.get_value<std::string>().c_str()));
            }
        }
    }
    ...
    double total = 0;
    for(auto & the_price : price){
        total += the_price;
    }

    double result = 0;
    for(int i = 0; i < price.size(); ++i){
        result += (price[i]/total)*impvol[i];
    }
