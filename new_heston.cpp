...
std::string OPURL(std::string ticker, std::string side, std::string expiration){
    std::string token = "";
    std::string url = "https://api.marketdata.app/v1/options/chain/" + ticker + "/?token=" + token + "&expiration=" + expiration + "&side=" +side;
    return url;
}
...
{
    std::string key = "";
    return "https://api.polygon.io/v2/aggs/ticker/" + ticker + "/range/1/day/2021-01-22/2024-07-25?adjusted=true&sort=asc&limit=1000&apiKey=" + key;
}
...
std::vector<double> Computer(std::vector<double> prices, double dt){
    ...
    double theta = mean(sim_vol);
    double sigma = variance(sim_vol);
...
double ImpliedVolatility(std::string ticker, std::string expiration){
    std::string call_response, put_response;
    call_response = GET(OPURL(ticker, "call", expiration));
    put_response = GET(OPURL(ticker, "put", expiration));

    ptree Calls = JSON(call_response);
    ptree Puts = JSON(put_response);

    std::vector<double> price, ivol;
    double total = 0;
    double weighted_iv = 0;

    for(ptree::const_iterator it = Calls.begin(); it != Calls.end(); ++it){
        if(it->first == "last"){
            for(ptree::const_iterator jt = it->second.begin(); jt != it->second.end(); ++jt){
                price.push_back(atof(jt->second.get_value<std::string>().c_str()));
            }
        }
        if(it->first == "iv"){
            for(ptree::const_iterator jt = it->second.begin(); jt != it->second.end(); ++jt){
                ivol.push_back(atof(jt->second.get_value<std::string>().c_str()));
            }
        }
    }

    for(ptree::const_iterator it = Puts.begin(); it != Puts.end(); ++it){
        if(it->first == "last"){
            for(ptree::const_iterator jt = it->second.begin(); jt != it->second.end(); ++jt){
                price.push_back(atof(jt->second.get_value<std::string>().c_str()));
            }
    ...
    for(int i = 0; i < price.size(); ++i){
        weighted_iv += (price[i]/total)*ivol[i];
    }
    
    return pow(weighted_iv, 2);
}
...
int main()
{
    ...
    std::string expiration = "2024-08-02";
    ...
    double iv = ImpliedVolatility(ticker, expiration);
